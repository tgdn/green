...
/***/ function(module, exports, __webpack_require__) {
	...
	var RadioCheckboxInput = function (_Input) {
	    _inherits(RadioCheckboxInput, _Input);

	    function RadioCheckboxInput(el) {
	        _classCallCheck(this, RadioCheckboxInput);

	        var _this3 = _possibleConstructorReturn(this, Object.getPrototypeOf(RadioCheckboxInput).call(this, el));

	        _this3.checked = _this3.$el.is(':checked');
	        return _this3;
	    }

	    _createClass(RadioCheckboxInput, [{
	        key: 'setupCallbacks',
	        value: function setupCallbacks() {
	            var _this4 = this;

	            this.$el.change(function (e) {
	                _this4.checked = $(e.target).is(':checked');
	                _this4.handleEvent(e);
	            });
	        }
	    }]);

	    return RadioCheckboxInput;
	}(Input);

	var MemberCheckbox = function (_RadioCheckboxInput) {
	    _inherits(MemberCheckbox, _RadioCheckboxInput);

	    function MemberCheckbox(el) {
	        _classCallCheck(this, MemberCheckbox);

	        var _this5 = _possibleConstructorReturn(this, Object.getPrototypeOf(MemberCheckbox).call(this, el));

	        _this5.inputcost = _this5.$el.parent('label').parent('.checkbox').find('.cost-container > input');
	        return _this5;
	    }
	...
	var BillForm = function () {
	    function BillForm() {
	        ...
	        this.form.find('input[name=billopt]').map(function (index, el) {
	            var billoptinput = new RadioCheckboxInput(el);
	            billoptinput.addHandler(_this6.onOptionChange);

	            that.inputs['opts'].push(billoptinput);
	        });

	        this.form.find('input[name=userselect]').map(function (index, el) {
	            var input = new MemberCheckbox(el);
	            input.addHandler(_this6.onMemberSelected);

	            that.inputs['members'].push(input);
	            if (input.checked) {
	                _this6.selected_members.push(input);
	            }
	        });
	    ...
	    _createClass(BillForm, [{
	        ...
	        value: function _bind() {
	            ...
	            methods.forEach(function (method) {
	                _this7[method] = _this7[method].bind(_this7);
	            });
	        }
	    }, {
	        key: 'onOptionChange',
	        value: function onOptionChange(el, event) {
	            this.opt = event.target.value;

	            if (this.opt == 'select') {
	                this.selected_members.forEach(function (el) {
	        ...
	        key: 'onMemberSelected',
	        value: function onMemberSelected(el, event) {
	            var checked = el.$el.is(':checked') ? true : false;
	            ...
	            if (checked) {
	                ...
	                this.selected_members = _underscore2.default.without(this.selected_members, el);
	            }

	            _underscore2.default.uniq(this.selected_members);

	            this.calculate();
	        }
	    }, {
	        key: 'onNameChange',
	        value: function onNameChange(el, event) {}
	    }, {
	        key: 'onCostChange',
	        value: function onCostChange(el, event) {
	            this.calculate();
	        }
	    }, {
	        key: 'calculate',
	        value: function calculate() {
	            var members_count = this.selected_members.length;
	            var cost = this.inputs['cost'].value;

	            if (members_count == 0) return;
	            if (cost == NaN || cost == null || cost.length == 0) cost = 0;

	            var cost_per_member = (cost / members_count).toFixed(2);
	        ...
	        value: function validate(event) {
	            ...
	            if (this.opt == 'select') {
	                ...
	                this.selected_members.some(function (el) {
	                    var tempval = parseFloat(el.inputcost.val());
	        ...
	        value: function handleSubmit(event) {
	            ...
	            var errors = this.validate(event);
	            var errorsBlock = $('#js-errors');
	            ...
	            $.post(url, data, function (resp) {
	                _this8.submitBtn.attr('disabled', false);
	                _this8.submitBtn.val(_this8.submitBtnVal);
